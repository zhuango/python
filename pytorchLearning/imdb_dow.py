...
class Model(nn.Module):
    def __init__(self, max_words, emb_size, hid_size, dropout):
        ...
        self.fc1_weight = Parameter(torch.Tensor(np.random.normal(0, 0.1, (self.hid_size, self.hid_size))))
        self.fc1_thr = Parameter(torch.Tensor(np.random.normal(0, 0.1, (self.hid_size, self.hid_size))))
        self.fc1_bias = Parameter(torch.Tensor(np.random.normal(0, 0.0, self.hid_size)))

        # self.fc2 = nn.Linear(self.hid_size, 2)
        self.fc2_weight = Parameter(torch.Tensor(np.random.normal(0, 0.1, (self.hid_size, 2))))
        self.fc2_thr = Parameter(torch.Tensor(np.random.normal(0, 0.1, (self.hid_size, 2))))
        self.fc2_bias = Parameter(torch.Tensor(np.random.normal(0, 0.0, 2)))
    ...
    def forward(self, x):
        ...
        # thr = torch.tanh(self.fc1_thr+self.fc1_weight)
        # real_w = self.fc1_weight * F.relu(self.fc1_weight * (1 + thr)).to(torch.bool).to(torch.float)
        # print(torch.sum(real_w.to(torch.bool).to(torch.float)))
        # x = torch.matmul(x, real_w) + self.fc1_bias
        
        x = F.avg_pool2d(x, (x.shape[1], 1)).squeeze()

        thr = torch.tanh(self.fc2_thr+self.fc2_weight)
        real_w = self.fc2_weight * F.relu(self.fc2_weight * (1 + thr)).to(torch.bool).to(torch.float)
        print(torch.sum(real_w.to(torch.bool).to(torch.float)))
        x = torch.matmul(x, real_w) + self.fc2_bias

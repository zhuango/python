...
class LeNetConvPoolLayer(object):
    ...
    def __init__(self, rng, input, filter_shape, image_shape, poolsize=(2, 2)):
        ...
        array = numpy.zeros(filter_shape, dtype=theano.config.floatX)
        array[0][0][0][0] = 1
        array[1][0][0][1] = 1
        # array[2][0][0][2] = 1
        ...
        # self.W = theano.shared(
        #     numpy.asarray(
        #         rng.uniform(low=0.0, high=0.0, size=filter_shape),
...
def preprocess(vectorFilePath, labelFilePath, dataSetPath, wordDimension, batch_size = 200):
    ...
    #############################################
    AddVectorCountToTimesOfBatch_size(vectorFilePath, batch_size, wordDimension)
    AddLabelCountToTimesOfBatch_size(labelFilePath, batch_size)
    #############################################
    train_vec_file=open(vectorFilePath, 'r')
    ...
    test_vec_file = open(vectorFilePath, 'r')
...
def SingleProcess(CorpusPath, CnnOutputPath, wordDimension, language, clas, corpusType):
    ...
    cnnOutputPath = CnnOutputPath + corpusType + "WOnlyOne/"

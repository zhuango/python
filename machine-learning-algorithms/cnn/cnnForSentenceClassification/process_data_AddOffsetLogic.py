...
def build_data_cv(data_folder, clean_string=True):
    ...
    train_offset_file = data_folder[2]
    test_context_file = data_folder[3]
    test_label_file = data_folder[4]
    test_offset_file= data_folder[5]
    ...
    with open(train_context_file, "r") as f:
        ...
        train_offsets = open(train_offset_file, 'r')
        for line in f:
            #print(line + "######################################################")
            label = train_label.readline().strip();
            offsets = train_offsets.readline().strip();
            rev = []
            ...
            orig_rev = ""
            for offset in offsets.strip().split(" "):
                #print(orig_rev_list[i])
                orig_rev_list[i] += "^"+ offset
                orig_rev += orig_rev_list[i] + " "
                i+=1
            orig_rev = orig_rev.strip()
            words = set(orig_rev_list)
            for word in words:
                vocab[word] += 1
            polarity = 0
            if label == posTag:
    ...
    with open(test_context_file, "r") as f:
        ...
        test_offsets = open(test_offset_file, 'r')
        for line in f:
            print(line.strip() + "######################################################")
            label = test_label.readline().strip();
            offsets = test_offsets.readline().strip();
            rev = []
            rev.append(line.strip())
            if clean_string:
                orig_rev = clean_str(" ".join(rev))
...
def load_vec_withOffset(fname, offsetVectorFileName, vocab):
    """
    format: word vec[50]
    """
    ...
    with open(fname, "rb") as f:
        for line in f:
            strs =line.strip().split(' ')
            word_dict[str.lower(strs[0])] = np.array([float(elem) for elem in strs[1:]], dtype='float32')
    
    offset_dict ={}
    with open(offsetVectorFileName, "rb") as f:
        for line in f:
            strs =line.strip().split(' ')
            offset_dict[str.lower(strs[0])] = np.array([float(elem) for elem in strs[1:]], dtype='float32')

    word_vecs = {}
    for word in vocab:
        wordItems = word.split("^")
        wordStr = wordItems[0]
        offsets = wordItems[1].split("&")

        word_vecs[str.lower(wordStr)] = np.concatenate((word_dict[wordStr], offset_dict[offsets[0]], offset_dict[offsets[0]]))
...
if __name__=="__main__":
    ...
    TraiOffsetFile = inputInfo["TraiOffset"]
    TestOffsetFile = inputInfo["TestOffset"]

    TraiLabelFile = inputInfo["TraiLabel"]
    TestLabelFile = inputInfo["TestLabel"]

    offsetVectorFile = inputInfo["OffsetVector"]
    ...
    data_folder = [TraiContextFile, TraiLabelFile,TraiOffsetFile, TestContextFile, TestLabelFile, TestOffsetFile]
    print "loading data...",
    ...
    w2v = load_vec_withOffset(w2v_file, offsetVectorFile, vocab)
    print "word2vec loaded!"
    print "num words already in word2vec: " + str(len(w2v))
    add_unknown_words(w2v, vocab, k = k + offsetLegnth * 2)
    #generate_simple_dict(w2v, wordVectorFile + ".simple")
    W, featureWordMap, word_idx_map = get_W(w2v, featureWordList, k + offsetLegnth * 2)
    rand_vecs = {}
    add_unknown_words(rand_vecs, vocab, k = k + offsetLegnth * 2)
    W2, _, _ = get_W(rand_vecs, featureWordList, k + offsetLegnth * 2)
